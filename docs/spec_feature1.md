# spec.md ΓÇö Feature 1: Multi-File PDF Upload & Job Queueing

**Satisfies:** FR1  
**Phase:** 1 ΓÇö Ingestion Pipeline  
**Status:** Draft
...
2. Validates each submitted file server-side (type and size) before a job record is created. A file that fails validation is rejected with a descriptive error message ΓÇö it is never silently dropped and never partially accepted.
...
- **Does not support file formats other than PDF.** No `.docx`, `.xlsx`, `.html`, or image files ΓÇö even if they contain financial statements.
- **Does not authenticate or authorize the uploader.** MVP is single-user, single-session by design (CONSTITUTION ┬º6.10). No login, no role check, no per-user job isolation.
...
- **Does not provide a progress indicator for the extraction step itself.** Status transitions (`queued` ΓåÆ `extracting` ΓåÆ `done` / `failed`) are surfaced, but Feature 1 does not implement real-time progress within an active extraction.
- **Does not send file content to any remote service.** File bytes are written to local storage only. Groq API is not touched at this stage (CONSTITUTION ┬º6.5).
...
2. **Invalid file type rejected with a clear error.** Submitting a non-PDF file (e.g., `.docx`, `.png`, `.xlsx`) results in a visible, named error per file (e.g., "resume.docx ΓÇö unsupported file type"). The invalid file is not added to the job list. Valid files submitted alongside it are not affected.
...
6. **Target metric selection recorded before queuing.** If the user changes the target metric selector before submission, the recorded `target_metric` reflects that selection ΓÇö not the default. The metric cannot be changed after the job leaves `queued` status.

7. **Job list visible and accurate.** The job list UI shows all jobs from the current session with their current `status`. A page refresh does not lose job records ΓÇö they survive via backend persistence.
...
| EC-5 | User closes the browser tab while jobs are queued. | Already-enqueued jobs continue processing server-side. Their records survive and are visible on reload (see AC-7). Jobs still in `queued` status at the frontend that were never sent to the server are lost ΓÇö this is acceptable at MVP. |
