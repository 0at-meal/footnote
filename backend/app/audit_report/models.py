...
class ReportMetadata(BaseModel):
    ...
    filing_year: int | None = Field(
        default=None, description="Filing year if identified"
    )
    target_metric: str = Field(
        ..., description="Target financial metric (e.g. Adjusted EBITDA)"
    )
    generated_at: str = Field(
        ..., description="ISO 8601 UTC timestamp of report compilation"
    )
    total_cells: int = Field(
        default=0, ge=0, description="Total cells generated in the workbook"
    )
    automated_count: int = Field(
        default=0, ge=0, description="Count of auto-accepted inputs"
    )
    verified_count: int = Field(
        default=0, ge=0, description="Count of human-verified/locked inputs"
    )
    flagged_count: int = Field(default=0, ge=0, description="Count of flagged inputs")
    override_count: int = Field(
        default=0, ge=0, description="Count of manual overrides/edits"
    )
...
class ReconciliationSummaryItem(BaseModel):
    ...
    sheet_name: str = Field(
        ..., description="Worksheet containing this reconciliation item"
    )
    cell_coord: str = Field(
        ..., description="A1-style cell reference (e.g. 'B2', 'B8')"
    )
    label: str = Field(..., description="Original or display line item label")
    normalized_label: str | None = Field(
        default=None, description="Standardized taxonomy label"
    )
    formula_expression: str | None = Field(
        default=None, description="Formula representation"
    )
    computed_value: str = Field(..., description="Formatted or computed value string")
    is_formula: bool = Field(
        default=False, description="True if cell contains a dynamic formula"
    )
    is_hardcode: bool = Field(
        default=False, description="True if tagged as a manual hardcode"
    )
...
class ProvenanceMatrixItem(BaseModel):
    ...
    computed_value: str = Field(
        ..., description="Computed value or raw extracted value"
    )
...
class ManualOverrideItem(BaseModel):
    ...
    item_id: str = Field(
        ..., description="Unique review item ID or coordinate reference"
    )
    ...
    original_value: str | None = Field(
        default=None, description="Original value before human edit"
    )
    final_value: str = Field(..., description="Final confirmed/locked value")
    original_label: str | None = Field(
        default=None, description="Original label before human edit"
    )
    final_label: str = Field(..., description="Final confirmed/locked label")
    review_status: str = Field(
        ..., description="Review status (e.g. locked, manual_required)"
    )
    confidence_band: str | None = Field(
        default=None, description="Extraction confidence band"
    )
    flags: list[str] = Field(
        default_factory=list, description="Diagnostic extraction flags"
    )
    error_detail: str | None = Field(
        default=None, description="Error detail if recovered from error"
    )
    confirmation_timestamp: str | None = Field(
        default=None, description="Human confirmation timestamp if recorded"
    )
    is_hardcode: bool = Field(
        default=False, description="True if generated as manual hardcode"
    )
...
class ClassifierAuditEntry(BaseModel):
    ...
    structural_context: str | None = Field(
        default=None, description="Structural context sent"
    )
    output_label: str | None = Field(
        default=None, description="Candidate label returned"
    )
    confidence: float | None = Field(
        default=None, description="Confidence score returned"
    )
...
class ClassifierAuditSummary(BaseModel):
    ...
    matched_count: int = Field(
        default=0, ge=0, description="Count of matched taxonomy labels"
    )
    pending_count: int = Field(
        default=0, ge=0, description="Count of unrecognized labels"
    )
    error_count: int = Field(
        default=0, ge=0, description="Count of classifier call errors"
    )
...
class DriftAuditSummary(BaseModel):
    ...
    is_evaluated: bool = Field(
        default=False, description="True if drift detection was run"
    )
    is_baseline: bool = Field(
        default=False, description="True if baseline initialization year"
    )
    has_discrepancy: bool = Field(
        default=False, description="True if redefinition detected"
    )
    ...
    prior_node_id: str | None = Field(
        default=None, description="Prior-year graph node ID compared against"
    )
    summary_text: str = Field(
        default="", description="Human-readable drift status summary"
    )
...
class CompiledAuditDataset(BaseModel):
    ...
    reconciliation_summary: list[ReconciliationSummaryItem] = Field(
        default_factory=list
    )
    ...
    has_manual_overrides: bool = Field(
        default=False, description="True if manual overrides exist"
    )
